...
def winner
  (player, numLosses) = losses.find {|player, losses| losses < MAX_LOSS_COUNT}
  return player
end
...
def update_standings
  ...
  if losses[previous_player] == MAX_LOSS_COUNT - 1
    puts "#{previous_player} has neem eliminated!"
    sleep (1)
  end
...
def take_turn
  ...
  add_letter(letter)
  puts "#{current_player} added the latter '#{letter}' to the fragment."
end
